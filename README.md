...
Composer・Node.js は不要です。QRコード生成・メール送信・グラフ描画も外部ライブラリを使わずに実装しています。
...
動作確認は「全体アンケート」ページの**テスト送信**か、次のコマンドで行えます。
...
管理画面の「全体アンケート」ページには、現在どの方式で送るかが表示されます。
...
10. イベント終了後、状態を**「終了」**にし、「全体アンケート」ページから案内メールを送ります。
...
C:\xampp\php\php.exe tests\http_test.php --force          REM E2E（153項目）
```
...
壁紙のアップロードとダウンロード・全体アンケートのトークン・メール送信方式の切り替えと本文の組み立て・
...
- **交換の照会は、来場者のQRコードをスタッフのスマホカメラで読み取るのが基本**です。
  QRには交換コードのURL（`/c/<コード>`）が入っていて、**ログイン済みの受付・主催者が読むと照会画面**、
  それ以外（来場者本人）が読むと**自分の回答済み画面**が開きます。専用のスキャナーアプリは不要で、
  OSの標準カメラで読み取れます。読み取れないときは、従来どおり交換コードを手入力できます。
