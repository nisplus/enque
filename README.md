...
C:\xampp\php\php.exe tests\http_test.php --force          REM E2E（251項目）
...
メールアドレスの確認表示（回答画面は本人が直せるようそのまま、回答済み画面は伏せ字）と変更・
...
- **来場者の同定は端末の匿名Cookieだけで、メールアドレスは必須にしていません。**
  ...
  スタッフが判断する運用です。
- **預かったメールアドレスは、本人の端末にだけ見せます。**
  2社目以降の回答画面には登録済みのアドレスを埋めて出し（打ち間違いに気づいて直せるように）、
  回答済み画面は受付にQRを見せる画面なので `ta***@example.jp` の形に伏せて出します。
  受付・企業担当・主催者の画面とCSVには、どこにもアドレスを出しません（件数のみ）。
  変更すると、まだ送っていない案内の宛先も新しいアドレスに揃えます。
